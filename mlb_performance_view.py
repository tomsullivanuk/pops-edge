...
def collection(assets, observation=None, download=None):
    records=[]
    ...
    if observation is not None:records=[('',observation)]
    ...
    href=download or '/performance/mlb/saved/'+name
    return body+'<a download href="'+text(href)+'">Download saved collection observation</a></details></section>'


def render(reader, assets, state, query, *, updates=None, update_status=None, controls=""):
    ref=state['live']
    if not ref:
        return page('mlb','<section class="panel"><div class="summary-heading"><h2>MLB Performance</h2>'+controls+'</div><p>No saved live Performance Report is selected.</p></section>')
    ...
    updated=label(generated)+(', '+str(generated.year) if generated.year!=end.year else '')+', '+generated.strftime('%I:%M %p').lstrip('0')+' CT'
    body+='<p class="muted summary-dates">Cumulative · '+text(period)+' <span>· Updated '+text(updated)+'</span></p>'
    ...
    body+='<p class="counts">'+' &nbsp; '.join('<strong>'+str(len(projection[key]))+'</strong> '+label for key,label in [('eligible_ids','Eligible'),('captured_ids','Captured'),('scored_ids','Scored')])+('<span class="coverage-note"> — Limited coverage</span>' if len(projection['scored_ids'])<len(projection['eligible_ids']) or report.coverages[0].unknown_calendar_dates else '')+'</p>'
    cats=projection['reconciliation'];count=lambda keys:sum(len(cats.get(k,[])) for k in keys)
    breakdown='<p>'+str(count(['missed_window','archive_unavailable','candle_unavailable']))+' missing captures · '+str(count(['acquisition_failed','captured_invalid','timing_invalid','archive_invalid','candle_invalid','derivation_unavailable']))+' unusable or failed · '+str(count(['outcome_unresolved']))+' awaiting result</p>'
    ...
    body+='</tbody></table></div><p>95% sampling interval: <strong>'+number(perf.uncertainty.lower)+'–'+number(perf.uncertainty.upper)+'</strong></p>'
    ...
    observation=None;download=None
    if updates and update_status:
        try:
            observation=updates.observation(update_status,ref['package_id'])
            if observation:download='/performance/mlb/update/evidence/'+update_status['attempt_id']+'/collection.json'
        except (OSError,ValueError,KeyError):
            pass  # Corrupt optional observation is unavailable; science is retained.
    body+='</section>'+match_table(reader.root,ref,analysis,projection,query,spec)+collection(assets,observation,download)
    ...
    body+='<p>Update report uses already collected research data. Missing captures remain in coverage; they are not scored as incorrect predictions.</p>'
    body+='<h3>Coverage</h3>'+breakdown+'<table><thead><tr><th>Category</th><th>Opportunities</th></tr></thead><tbody>'
