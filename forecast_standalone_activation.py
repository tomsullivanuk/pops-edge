...
def publish_supporting_session_completion(*,archive:NamespaceArchive,session_id:str,requested_date_window:tuple[str,str],command_started_at:datetime,provider_calls:int,cutoff_at:datetime,mlb_acquisition:Mapping[str,Any],kalshi_acquisition:Mapping[str,Any])->str:
    ...
    for provider,acquisition in (("mlb-stats-api",mlb_acquisition),("kalshi",kalshi_acquisition)):
        ...
        if value.get("family")!="refresh-retrospective-supporting" or value.get("provider")!=provider or not value.get("acquisition_id","").startswith(session_id+":"):raise OperationsError("supporting-session-conflict",f"{provider} acquisition is foreign")
...
def complete_supporting_session_from_archive(*,archive:NamespaceArchive,session_id:str)->Mapping[str,Any]:
    ...
    dates=tuple(date.fromisoformat(value["request_identity"]) for _,value in mlb);expected=tuple(dates[0]+timedelta(days=offset) for offset in range((dates[-1]-dates[0]).days+1))
    ...
    def page_dt(value,name):return datetime.fromisoformat(value[name]["datetime_utc"]) if isinstance(value[name],dict) else datetime.fromisoformat(value[name])
    ...
    for global_position,(entry,value) in enumerate(sorted(catalog,key=lambda pair:({"historical":0,"live":1}.get(pair[1].get("partition"),2),pair[1].get("partition_position",-1)))):
        ...
        raw=archive.read_verified("raw",entry["raw_object_sha256"]);payload=json.loads(raw);typed_catalog.append(KalshiCatalogPage(global_position,cursor,payload["cursor"],raw,tuple(payload["markets"]),page_dt(value,"started_at"),page_dt(value,"completed_at"),value["endpoint"],partition,position))
    mlb_pages=[]
    ...
    return {**result,"provider_calls":0,"completed_session_id":session_id}
