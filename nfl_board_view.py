...
def render(data):
    ...
    for item in rows:
        ...
        if data.get('accounting_enabled'):
            ...
            if any(t['ticker'] in problem_tickers or t['needs_review'] for t in unresolved):
                wager_cell+='<small class="reason">Accounting review needed · see calculation notes</small>'
    ...
    if data.get('accounting_enabled'):
        headers[7]=('Recorded trades / closed cost','Recorded trade direction, quantity, export YES-scale price, fee and date; separately reconciled closed-position cost. Not current holdings.')
    ...
    if data.get('accounting_enabled'):
        activity_note='<p class="activity-note">Recorded trades are transaction records, not confirmed current holdings. Export prices use the YES scale, including NO-direction trades; they are not labeled as NO purchase prices. Purchase/sale direction is not inferred. Reconciled closed-position cost and realized proceeds/profit remain separate. Absence of a closed P&amp;L record alone is not an accounting error.</p>'
        activity_note+=''.join('<p>'+esc(d['ticker'])+' · Recorded trade; no closed P&amp;L record. Current holding unverified.</p>' for d in data.get('accounting_notes', []))
    ...
    trade_guide=('Recorded trades show export observations, not a confirmed position or purchase/sale classification. Prices in those records are on the export YES scale. Closed-position cost includes opening fees; proceeds and profit require reconciliation. No open-position totals or expected payouts are inferred.' if data.get('accounting_enabled') else 'Wagered includes recorded purchase fees. Expected payout is the model-weighted gross return on the recorded quantity, including tie outcomes; the smaller “if win” figure is the gross return if that row’s team wins. These are outcome-level totals for each game, assuming purchases still held. They are not confirmed open balances.')
...
<div class="guide"><p><b>Difference after fee = ELWAY Contract − Kalshi price − estimated one-contract fee.</b> ELWAY Contract is expected payout per contract: $1.00 on a win, $0.00 on a loss and $0.50 on a tie. {trade_guide} Payout includes returned stake; it is not profit. Dollars display to two decimals; calculations retain full precision. Original win probabilities and before-fee differences remain in Details.</p>
...
<p>{'File creation time — publication-time proxy (publisher time unknown)' if data.get('forecast_time_basis') else 'ELWAY updated'}: {esc(display_time(data['forecast_updated_at']))}<br>Verified: {esc(display_time(data['forecast_verified_at']))}<br>Schedule received: {esc(display_time(data['schedule_received_at']))}<br>Kalshi capture: {esc(display_time(data['capture_started_at']))} to {esc(display_time(data['capture_completed_at']))}<br>Snapshot built: {esc(display_time(data['generated_at']))}</p>
