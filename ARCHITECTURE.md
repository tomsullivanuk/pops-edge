...
## v1.3 Part 2 — Performance presentation

The existing local server gains separate read-only NFL and MLB reader adapters and shared area navigation. NFL consumes replay-validated saved weekly reports; MLB exposes permitted assets of the established saved entry with retained package checks. No shared scientific schema, new scoring authority or provider action is introduced. See [contract](docs/PERFORMANCE_READERS_v1.3.md).
...
## MLB v1.2 report delivery — documentation-stage successor

The [reporting contract](docs/MLB_REPORTING_CONTRACT_v1.2.0.md) defines a bounded,
archive-read-only analytical path and static reader. Its dependency is source
Evidence → canonical validated Analysis/report package → rendered Performance
Report. Methodology governs separate source-cutoff, observation-window and actual
computation/report times. A typed, versioned `StandaloneReportContext` carries
those meanings and exact source pins. Legacy V3 graph/record meanings remain
unchanged; successor analytical dispatch is used only where the old shared
`analysis_boundary` cannot express the new context exactly.

Replay first verifies complete pinned source authority through the evidence cutoff,
then derives the complete population and report-local Analysis with truthful
computation provenance. It does not bypass old checks or admit arbitrary post-cutoff
...
package is available. A separate attempt-status record exposes failed generation
while retaining prior report references/dates. The historical package has an independent
static link and is not regenerated by normal live-quote updates. Corrupt/ambiguous
source authority fails closed; missing operational status is independently visible.

The existing singleton retrospective publication, its stage and archive schema are
not expanded or reset for this path. No report operation modifies source Evidence,
indexes, projections, capture markers, scheduled jobs or collection state. Status
presentation consumes existing sanitized material read-only. Source pinning must
handle concurrent appends without creating a production mutation requirement.
No server, shared application shell, queue, generic version store or recovery
...
The Owner authorized the [NFL-style MLB reader](docs/MLB_PERFORMANCE_READER_v1.3.md), including cumulative Kalshi/reference summary, complete retained opportunity rows, Period/Custom Period and Team display filters, Central dates, compact Details and bottom historical link. It supersedes the earlier iframe-presentation limitation and redesign deferral for this bounded slice. Scientific packages, scoring, populations and collector authority remain unchanged. Explicit manual match-display preparation is separate from browser reads. Implementation and independent review are authorized; integration/deployment/release are separate gates.
