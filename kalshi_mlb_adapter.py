...
ALIASES={"d-backs":"arizona diamondbacks","diamondbacks":"arizona diamondbacks","a's":"athletics","oakland athletics":"athletics","white sox":"chicago white sox","red sox":"boston red sox"}
...
def reconcile_market(market:dict,games:tuple[MLBGame,...],schedules:tuple[ScheduleObservation,...]):
    ...
    schedule_by_event={x.canonical_event_id:x for x in schedules}; candidates=[]
    for game in games:
        ...
        if participant and (not date_text or (schedule and schedule.scheduled_start.date().isoformat()==date_text)): candidates.append((game,participant))
...
def _settlement_evidence(market):
    ...
    if isinstance(raw,list): return tuple(str(x) for x in raw)
    return ()

def adapt_market(market:dict,*,games:tuple[MLBGame,...],schedules:tuple[ScheduleObservation,...],collected_at:datetime,orderbook_payload:dict|None=None,orderbook_collected_at:datetime|None=None)->MarketAdapterResult:
    ...
    if not no_label or _canon(no_label)!=_canon(other.display_name):
    ...
    if not any(marker in settlement_text for marker in ("$1","one dollar","100 cents")):
    ...
    endpoint="/markets"; components=[ComponentEvidence("metadata-quotes",market_id,endpoint,collected_at,raw,_digest(market))]
    book=()
    ...
    prov=Provenance("kalshi",market_id,game.event.canonical_event_id,collected_at,transformations=("parsed-provider-market","complemented-resting-bids-when-direct-asks-absent") if orderbook_payload else ("parsed-provider-market",),raw_evidence_ref=raw,source_url=BASE+f"/markets/{market_id}",collector_version=ADAPTER_VERSION)
    series=ProviderMarketSeries(f"market-series:kalshi:{market_id}","kalshi",market_id,market["ticker"],proposition.proposition_id,SideSemantic(MarketSide.YES,proposition.proposition_id,participant_id,True,str(market.get("yes_sub_title") or market.get("yes_label"))),SideSemantic(MarketSide.NO,proposition.proposition_id,other.canonical_team_id,False,str(no_label)),str(market["title"]),market.get("subtitle") or market.get("sub_title"),market.get("event_ticker"),_dt(market.get("open_time")),_dt(market.get("close_time")),settlement,prov)
    obs_seed=f"{market_id}|{collected_at.isoformat()}|{raw}|{_digest(orderbook_payload) if orderbook_payload else ''}"
    try: obs=MarketObservation("market-observation:"+hashlib.sha256(obs_seed.encode()).hexdigest(),series.series_id,market_id,game.event.canonical_event_id,proposition.proposition_id,"capture:"+hashlib.sha256(obs_seed.encode()).hexdigest(),collected_at,status,quote,book,tuple(components),prov,_dec(market.get("volume_fp") or market.get("volume")),_dec(market.get("open_interest_fp") or market.get("open_interest")),tuple(local_issues))
