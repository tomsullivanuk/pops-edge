...
def main(argv=None):
    ...
    for command in ('activate-display','rollback-display'):
        ...
        else:item.add_argument('--operational-state',type=Path,help='existing sanitized invocation directory; read only')
    ...
    try:
        ...
        if args.command in {'activate-display','rollback-display'}:
            ...
            value=(activate_display(output=args.output,expected_revision=args.expected_revision,operational_state=args.operational_state) if args.command=='activate-display' else
