...
Deployment artifacts are produced only by an inert renderer that validates an activated configuration and writes absolute interpreter, application, and configuration paths; rendering never installs or loads jobs. Commands fail nonzero when provider adapters, credentials, archived authority, or readiness are absent. Sanitized operational heartbeats drive health state without becoming or modifying Evidence.

Initialization is an explicit offline command accepting only the committed prospective authority identity. Supporting refresh decodes MLB-owned event facts and current Kalshi catalog shapes through one fixture/live composition path; outcome reconciliation appends only changed MLB history. Public catalog and order-book GETs do not retrieve Keychain credentials. Health evaluates prospective, hourly, daily-maintenance, outcome, secondary, and health cadences independently.
PR17C1's proposed prospective population is every 2026 MLB regular-season scheduled-state opportunity at or after the activation boundary, including distinct doubleheader games and visibly unresolved postponed, rescheduled, suspended, resumed, cancelled, ambiguous, or invalid cases. Kalshi availability never controls MLB population membership: missing and ambiguous mappings remain visible per opportunity. Supporting authority is append-only and correction-aware; complete bounded cursor pagination and New York obligation-driven MLB date selection prevent silent catalog or schedule truncation. Provider YES/NO bids are normalized into complementary canonical NO/YES executable offers, with both sides required. These statements do not claim that activation, deployment, live access, or Evidence collection has occurred.

Exact provider pages and derived normalized unions are distinct: replay grants typed authority only through a complete acquisition manifest binding ordered page identities, bytes, digests, chronology, union rule, and contracts. Every unresolved non-terminal game remains an outcome obligation regardless of age; only resolved-game correction checks use the bounded lookback. A single trusted command-start instant fixes dates, and opaque Kalshi cursors are URL-encoded without permitting query injection.

Provider requests retain distinct trusted start/completion times even across Eastern midnight. Page-only or conflicting acquisition groups are visible unhealthy partial material and cannot enter replay, indexes, secondary authority, or maintenance until a deterministic create-only retry completes the same group.
...
## NFL presentation and source chronology

The full-season sheet composes immutable weekly captures and workbook-only
placeholders. Source times remain per game; the view does not imply one fresh
season-wide capture. Completion requires explicit final status or a supported
settlement, not elapsed kickoff. Only the newest selected activity export is
used; exports do not accumulate. Missing prices remain unavailable.

Freshness is evaluated when the season view is assembled. An open page does not
